...
def return_badge(val):
    ...
    if val >= 0: return f'<span class="metric-positive">+{val*100:.2f}%</span>'
...
def render_mode_tab(mode_data, mode_name):
    ...
    pick = top[0]
    ticker = pick['ticker']
    exp_ret = pick.get('expected_return', 0)
    jump_adj = pick.get('jump_adjustment', 0)
    jump_int = pick.get('jump_intensity', 0)

    st.markdown(f"""
    <div class="hero-card">
        <div style="font-size: 1.2rem; opacity: 0.8;">📈 {mode_name} TOP PICK (Jump‑Adjusted)</div>
        <div class="hero-ticker">{ticker}</div>
        <div>Expected Return: {return_badge(exp_ret)}</div>
        <div style="margin-top: 0.5rem;">Jump Adj: {return_text(jump_adj)} | λ: {jump_int:.2f}/yr</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for p in top:
        rows.append({
            'Ticker': p['ticker'],
            'Exp Return': return_text(p.get('expected_return', 0)),
            'Jump Adj': return_text(p.get('jump_adjustment', 0)),
            'λ (/yr)': f"{p.get('jump_intensity', 0):.2f}"
        })
    ...
    if universes:
        ...
        for t, d in universes.items():
            all_rows.append({
                'Ticker': t,
                'Exp Return': return_text(d.get('expected_return', 0)),
                'Jump Adj': return_text(d.get('jump_adjustment', 0)),
                'λ (/yr)': f"{d.get('jump_intensity', 0):.2f}"
            })
...
def render_shrinking_tab(shrinking_data):
    ...
    <div class="hero-card">
        ...
        <div>{shrinking_data['conviction']:.0f}% conviction · {len(shrinking_data.get('windows',[]))} windows</div>
    </div>
    """, unsafe_allow_html=True)
    with st.expander("📋 All Windows"):
        ...
        for w in shrinking_data.get('windows', []):
            rows.append({
                ...
                'ETF': w['ticker'],
                'Exp Return': return_text(w.get('expected_return', 0))
            })
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
...
st.markdown('<div>Merton Jump‑Diffusion – Macro‑Conditioned Jump Intensity</div>', unsafe_allow_html=True)
